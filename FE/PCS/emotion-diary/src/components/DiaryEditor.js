...
import { diaryIdState, diaryListState } from "./../store/atoms";
...
const DiaryEditor = ({ isEdit, originData }) => {
  ...
  const onRemove = (targetId) => {
    const newList = diaryList.filter((it) => it.id !== targetId);
  ...
  const onEdit = (targetId, date, content, emotion) => {
    ...
    const newList = diaryList.map((it) =>
      it.id === targetId ? { ...newItem } : it
    );
    setDiaryList(newList);
  };
